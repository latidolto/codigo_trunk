...
public class MenuConfig extends CommonManagedBean{
	...
	public List<Menu> getMenu1(){
		...
		return getFacade().getListFromParameters(Menu.class.getName(),"findMenuBySistem", params);
	}
	...
	public List<Menu> getMenu2(){
		...
		return getFacade().getListFromParameters(Menu.class.getName(),"findMenuChildren", params);
	}
	...
	public void saveMenuAction(ActionEvent ae) {
		...
		Sistema sis = (Sistema)getFacade().getEjb(Sistema.class.getName());
		Menu m = (Menu)getFacade().getEjb(Menu.class.getName());
		if(sis != null) {
			if(m.getEtiqueta() != null && m.getDescripcion() != null) {
				...
				getFacade().persistEjb(Menu.class.getName());
	...
	public void editMenu(Menu row) {
		...
		getFacade().setEjb(Menu.class.getName(), row);
	...
	public void saveEditMenuAction() {
		Menu m = (Menu)getFacade().getEjb(Menu.class.getName());
		if(m.getEtiqueta() != null && m.getDescripcion() != null) {
			getFacade().mergeEjb(Menu.class.getName());
			JsfUtils.sendMessageToView_INFO("Guardado Exitoso");
			JsfUtils.resfreshComponentById("panelMenu");
	...
	public void confirmDeleteMenuAction() {
		...
		getFacade().deleteEjb(Menu.class.getName());
	...
	public List<Tarea> getTareas(){
		...
		if(sis != null && men != null) {
			...
			lt = getFacade().getListFromParameters(Tarea.class.getName(),"findTareaByMenu", params);
		}
		return lt;
	}
	...
	public void addTareaAction(ActionEvent ae) {
		getFacade().setEjb(Tarea.class.getName(), new Tarea());
	}
	...
	public void onSelectTarea(Tarea tarea, String typeOfSelection, String indexes) {
		...
		getFacade().setEjb(Tarea.class.getName(), tarea);
	}
	...
	public void saveTareaAction() {
		...
		if(sis != null && men != null) {
			...
			if(t.getIdTarea() == 0) {
				getFacade().persistEjb(Tarea.class.getName());
			} else {
				getFacade().mergeEjb(Tarea.class.getName());
			}
			JsfUtils.sendMessageToView_INFO("Guardado Exitoso");
			JsfUtils.resfreshComponentById("panelTareas");
	...
	public void confirmDeleteTareaAction() {
		getFacade().deleteEjb(Tarea.class.getName());
		JsfUtils.sendMessageToView_INFO("Borrado Exitoso");
		JsfUtils.resfreshComponentById("panelTareas");
		java.lang.System.out.println("Borrado sin problemas.");
	}
	...
	public List<Menu> getMenuPrev(){
		Sistema sis = (Sistema)getFacade().getEjb(Sistema.class.getName());
		return getFacade().getMenu(true,sis.getIdSistema());
	}
